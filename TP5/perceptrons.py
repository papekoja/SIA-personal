import numpy as np
import perceptron_simple as s_p
...
class Perceptrons(object):
    ...
    def activation(self, init_input: np.ndarray, training: bool = False) -> np.ndarray:
        ...
        for layer in self.network:
            ...
            """Se transpone la matriz resultante para que las salidas de los perceptrones se conviertan en las entradas para la siguiente capa."""
    ...
    def __init_network(self, hidden_layout: [int], full_hidden: bool = False,
        ...
        for level in range(len(layout)):
            ...
            self.network[level] = np.empty(shape=layout[level], dtype=s_p.SimplePerceptron)
            ...
            for index in range(layout[level]):
                ...
                self.network[level][index] = \
                    s_p.SimplePerceptron(self.act_func, self.act_func_der, dim, hidden, index, momentum, mom_alpha)